...
def test_race_api():
    ...
    assert (
        "Dragonborn" in response.json()["results"][0]["name"]
    ), "Expected 'Dragonborn' in first race"
    assert (
        "Dwarf" in response.json()["results"][1]["name"]
    ), "Expected 'Dwarf' in second race"
...
def test_class_api():
    ...
    assert response.status_code == 200, "Expected status code 200"
    assert (
        "Barbarian" in response.json()["results"][0]["name"]
    ), "Expected 'Barbarian' in first class"
    assert (
        "Bard" in response.json()["results"][1]["name"]
    ), "Expected 'Bard' in second class"
...
def test_api_empty_response():
    ...
    ), "Expected no results in the API response"
