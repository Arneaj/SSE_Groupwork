...
def test_character_creation_page(client):
    ...
    assert (
        b"Choose race" in response.data
    ), "Expected 'Choose race' to be in the page content"
    assert (
        b"Choose a class" in response.data
    ), "Expected 'Choose a class' to be in the page content"
...
def test_calculate_ability_modifier():
    ...
    """
    assert (
        determine_ability_modifier(skill="stealth", ability_score=2, race="human") == -4
    )
    assert (
        determine_ability_modifier(skill="acrobatics", ability_score=7, race="elf")
        == -2
    )
    assert (
        determine_ability_modifier(skill="persuasion", ability_score=15, race="dwarf")
        == 2
    )
    assert (
        determine_ability_modifier(skill="intimidation", ability_score=20, race="orc")
        == 5
    )
...
def test_create_player_valid(monkeypatch, client, test_input, expected):
    ...
    response = client.post(
        "/save_player", data=json.dumps(test_input), content_type="application/json"
    )
    assert (
...
def test_create_player_missing_race(monkeypatch, client):
    ...
    mock_response.status_code = (
        404  # Assuming the API returns a 404 error for missing races
    )
...
def test_create_player_missing_class(monkeypatch, client):
    ...
    mock_response = Mock()
    mock_response.status_code = (
        404  # Assuming the API returns a 404 error for missing classes
    )
    monkeypatch.setattr(requests, "get", lambda url: mock_response)
