...
# MAY NEED TO IMPLEMENT DROP_ALL IN HERE so we can have nice stuff saved
# in our tables for the demo
# from .cli import create_all, drop_all, populate
# Importing commands - commented out as we didn't want to
# create/drop things every time at one point
...
def startGame():
    # Get the game from the request - game_id is separate
    # to the player id (primary key)
    game_id_input = request.args.get("game_id")

    # Get the game from the database
    passed_game = db.get_or_404(
        game_data, game_id_input, description="Found no such game in database"
    )
    ...
    passed_players = [
        db.get_or_404(
            player_data,
            player_id,
            description="At least one player missing from the database",
        )
        for player_id in passed_game.player_id
    ]

    return render_template(  # Takes user to the main page
                             # - containing info on game, players,
                             # abilities, monsters, dice
        "main.html",
        game=passed_game,
...
def create_game():
    ...
    # If no game name was provided, render the game creation form
    # (consider returning a 400 response)
        ...
        if isinstance(bonuses, list):
            for bonus in bonuses:
                ...
                if ability.lower() == skill.lower():
                    return bonus["bonus"]
    # If no modifier for that race and skill,
    # or if the request fails, return 0
    return 0
...
def calculate_hp(class_name, constitution_score, race):
    ...
    if response.status_code == 200:
        ...
        hit_die = response_specific_class.get("hit_die", 0)
...
def save_character():
    ...
    last_player_id_row = (
        db.session.query(
            player_data.id).order_by(
            db.desc(player_data.id)).first()
    )
